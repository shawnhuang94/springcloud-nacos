package com.nt.backend.workflow.controller.react;
...
public class ReactJsonToBpmnController {
    ...
    private static String create(String froworkflow, JSONObject flowNode,BpmnModel model,Process process,List<SequenceFlow> sequenceFlows) throws InvocationTargetException, IllegalAccessException {
        Integer nodeType = flowNode.getInteger("type");
        if (Type.PARALLEL.type.equals(nodeType)) {
            return createParallelGatewayBuilder(froworkflow, flowNode,model,process,sequenceFlows);
        }
        else if (Type.ROUTER.type.equals(nodeType)) {
            return createExclusiveGatewayBuilder(froworkflow, flowNode,model,process,sequenceFlows);
        }
        else if (Type.ROOT.type.equals(nodeType)) {
            flowNode.put("incoming", Collections.singletonList(froworkflow));
    ...
    private static String createExclusiveGatewayBuilder(String forworkflow, JSONObject flowNode,BpmnModel model,Process process,List<SequenceFlow> sequenceFlows) throws InvocationTargetException, IllegalAccessException {
        ...
        process.addFlowElement(connect(forworkflow, exclusiveGatewayId,sequenceFlows));
        ...
        if (Objects.nonNull(childNode)) {
            if (incoming == null || incoming.isEmpty()) {
                ...
                process.addFlowElement(BpmnModelUtils.createExclusiveGateWayEnd(endExId));
...
//        return exclusiveGatewayId;
    ...
    private static String createParallelGatewayBuilder(String forworkflow, JSONObject flowNode,BpmnModel model,Process process,List<SequenceFlow> sequenceFlows) throws InvocationTargetException, IllegalAccessException {
        String name = flowNode.getString("nodeName");
        ...
        process.addFlowElement(connect(forworkflow, parallelGatewayId,sequenceFlows));
    ...
    private static String createServiceTask(JSONObject flowNode,Process process,List<SequenceFlow> sequenceFlows) {
        ...
        if (incoming != null && !incoming.isEmpty()) {
            ...
            serviceTask.setImplementation("com.dingding.workflow.listener.ServiceListener");
