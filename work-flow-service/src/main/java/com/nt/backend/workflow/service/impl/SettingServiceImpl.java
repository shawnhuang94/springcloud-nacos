...
import com.nt.backend.workflow.entity.TemplateGroupBo;
...
public class SettingServiceImpl implements SettingService {
        ...
        coverMap.forEach((key, val) -> {
            ...
            val.forEach(v -> {
                if (ObjectUtil.isNotNull(v.getTemplateId())) {
                     ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionKey(PROCESS_PREFIX + v.getTemplateId()).latestVersion().singleResult();
                    String processDefinitionId = null;
                    if(processDefinition!=null){
                        processDefinitionId = processDefinition.getId();
                    }
                    templates.add(TemplateGroupVo.Template.builder()
                            .forworkflow(v.getTemplateId())
                            .tgId(v.getId())
                            .remark(v.getRemark())
                ...
                for (TemplateGroupVo.Template item : group.getItems()) {
                    templateGroupService.updateById(
                            TemplateGroup.builder()
                                    .id(item.getTgId())
                                    .groupId(group.getId())
                                    .templateId(item.getForworkflow())
                                    .sortNum(j + 1).build());
                    j++;
                }
                i++;
    ...
    public Object updateFormDetail(ProcessTemplates template) {
        ...
        ProcessTemplates processTemplates = processTemplateService.getById(template.getForworkflow());
        ...
        BpmnModel bpmnModel = assemBpmnModel(jsonObject, childNode, template.getRemark(),
            template.getFormName(), template.getGroupId(), template.getForworkflow());
    ...
    private BpmnModel assemBpmnModel(JSONObject jsonObject, ChildNode childNode, String remark,
        ...
        process.setId(PROCESS_PREFIX+templateId);
